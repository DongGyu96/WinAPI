...
#define STAGE_Y 520 // 스테이지의 y크기
#define SPEED 15 // 한틱당 이동거리
#define M_SPEED 5 // 작살 그림이 늘어나는 크기

#define STAGE_LIMIT 3 // 제한 스테이지 수
#define P_LIFE 3
#define STAGE1_LIMIT_TIME 6 // 스테이지1의 제한시간 (n9초)
#define STAGE2_LIMIT_TIME 5
#define STAGE3_LIMIT_TIME 5

#define WINCX 1024	//윈도우 크기
#define WINCY 768	//윈도우 크기
...
void P1_Animation(int, int, HDC, BOOL, BOOL);
void P2_Animation(int, int, HDC, BOOL, BOOL);
BOOL Shot1, Shot2; // 작살을 발사할때 애니메이션을 킬건지 말건지 결정하는 변수
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT IMessage, WPARAM wParam, LPARAM lParam)
{
	...
	static int M_height1[5], M_height2[5]; // 작살의 길이
	static int height1[5], height2[5]; // 작살 그림의 길이
	static RECT P1, P2; // 플레이어 위치
	static RECT rcTemp = { 0, 0, 0, 0 }; //충돌전용 RECT
	static RECT Missile_P1[5], Missile_P2[5]; // 2P니 난이도를 위해서 한발만 가지고 사용하기로
	static CImage img, blackimg, Logo, Key, Missile, timer, P1_face, P2_face, Number, Alphabet, Number2, Number3, Mitem, Life, DeathLife, img2, GameOver, Ending, img3;
	...
	static BOOL Move1, Move2; // 애니메이션을 돌릴지 말지 결정
	static BOOL Right1, Right2; // 캐릭터들의 방향
	static BOOL M1[5], M2[5]; // 작살 발사 여부
	static BOOL Stop = false, Stop_item = false;
	static BOOL T; // 시간이 다되었을때
	static BOOL M = false;
	static BOOL H; // 생명추가
	static BOOL P1_Play, P2_Play, P1_attack, P2_attack; // 게임플레이 가능/불가능, 피격판정
	static int P1_Life, P2_Life; // 목숨갯수
	static RECT timer_item; // 시간정지 아이템
	static RECT Missile_item, Life_item;
	static int P1_mCount, P2_mCount, P1_aCount, P2_aCount; // 플레이어 작살 최대갯수, 플레이어 피격판정 무적시간
	static HPEN hPen, oldPen;
	static int Time_item_count = 0; // 시간 정지 카운트
	static int Time_count, check_time; // 시간 카운트
	static int Time1_count;
	static int score1, score2, P1_score[5], P2_score[5];
	...
	case WM_CREATE:
		...
		DeathLife.LoadFromResource(g_hInst, MAKEINTRESOURCE(IDB_BITMAP44));
		...
		P1_face.LoadFromResource(g_hInst, MAKEINTRESOURCE(IDB_BITMAP36));
		P2_face.LoadFromResource(g_hInst, MAKEINTRESOURCE(IDB_BITMAP37));
		...
		Move1 = Move2 = Right1 = Right2 = P1_attack = P2_attack = false;
		T = false;
		Start = false;
		P1.top = STAGE_Y - 70;
		P1.left = 50;
		...
		P2.top = STAGE_Y - 70;
		P2.left = rt.right - 120;
		P2.right = rt.right - 50;
		P2.bottom = STAGE_Y;

		P1_Life = P2_Life = P_LIFE;
		P1_Play = true;
		P2_Play = false;

		P1_mCount = P2_mCount = 1;
	...
	case WM_TIMER:
		...
		case 1:
			...
			if (Start) // 엔터키를 누르고나서.
			{
				...
				{
					if (M1[i]) // 작살
						Missile.TransparentBlt(mem1dc, Missile_P1[i].left, Missile_P1[i].top, 30, M_height1[i], 0, 0, 30, height1[i], RGB(0, 0, 0));
					if (M2[i]) // 작살
						Missile.TransparentBlt(mem1dc, Missile_P2[i].left, Missile_P2[i].top, 30, M_height2[i], 0, 0, 30, height2[i], RGB(0, 0, 0));
				}

				if (P1_Play) // 플레이어1 활성화
				{
					if (P1_attack)
					{
						P1_aCount++;
						if (P1_aCount % 2 == 0)
							P1_Animation(P1.left, P1.top, mem1dc, Move1, Right1);
						if (P1_aCount == 30)
						{
							P1_aCount = 0;
							P1_attack = false;
					...
					P1_face.TransparentBlt(mem1dc, 0, STAGE_Y + 25, 150, 150, 0, 0, 150, 150, RGB(255, 0, 0));
					for (int i = 0; i < P_LIFE; ++i)
					{
						if (i < P1_Life)
							Life.TransparentBlt(mem1dc, 150 + (i * 50), STAGE_Y + 75, 50, 50, 0, 0, 30, 30, RGB(27, 24, 24));
						else
							DeathLife.TransparentBlt(mem1dc, 150 + (i * 50), STAGE_Y + 75, 50, 50, 0, 0, 30, 30, RGB(27, 24, 24));
					}
					// 플레이어들의 사용가능한 작살갯수 출력
					for (int i = 0; i < P1_mCount; ++i)
					{
						Mitem.StretchBlt(mem1dc, i * 30, 700, 30, 30, 0, 0, Mitem.GetWidth(), Mitem.GetHeight(), SRCCOPY);
					}
					for (int i = 0; i < 5; ++i)
					{
						Number2.TransparentBlt(mem1dc, 150 + (i * 30), STAGE_Y + 35, 30, 36, P1_score[i] * (Number2.GetWidth() / 10), 0, Number2.GetWidth() / 10, Number2.GetHeight(), RGB(255, 0, 255));
					}
				}
				else
				{
				...
				if (P2_Play) // 플레이어2 활성화
				{
					if (P2_attack)
					{
						P2_aCount++;
						if (P2_aCount % 2 == 0)
							P2_Animation(P2.left, P2.top, mem1dc, Move2, Right2);
						if (P2_aCount == 30)
						{
							P2_aCount = 0;
							P2_attack = false;
					...
					P2_face.TransparentBlt(mem1dc, rt.right - 150, STAGE_Y + 25, 150, 150, 0, 0, 150, 150, RGB(255, 0, 0));
					for (int i = 0; i < P_LIFE; ++i)
					{
						if (i<P2_Life)
							Life.TransparentBlt(mem1dc, (rt.right - 150) - ((i + 1) * 50), STAGE_Y + 75, 50, 50, 0, 0, 30, 30, RGB(27, 24, 24));
						else
							DeathLife.TransparentBlt(mem1dc, (rt.right - 150) - ((i + 1) * 50), STAGE_Y + 75, 50, 50, 0, 0, 30, 30, RGB(27, 24, 24));
					}
					for (int i = 0; i < P2_mCount; ++i)
					{
						Mitem.StretchBlt(mem1dc, rt.right - (30 * i) - 30, 700, 30, 30, 0, 0, Mitem.GetWidth(), Mitem.GetHeight(), SRCCOPY);
					}
					for (int i = 0; i < 5; ++i)
					{
						Number2.TransparentBlt(mem1dc, (rt.right - 150) - ((i + 1) * 30), STAGE_Y + 35, 30, 36, P2_score[i] * (Number2.GetWidth() / 10), 0, Number2.GetWidth() / 10, Number2.GetHeight(), RGB(255, 0, 255));
					}
				}
				else
				{
				...
				Move1 = false; // 별다른 키 입력이 없으면 애니메이션이 정지됨
				Move2 = false;
